...
class MainActivity : AppCompatActivity(), BluetoothServerManager.Listener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 2. 屏幕常亮 + 后台 CPU 保活（防止蓝牙硬件基带休眠）
        window.addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON)
        try {
            val pm = getSystemService(android.content.Context.POWER_SERVICE) as? android.os.PowerManager
            wakeLock = pm?.newWakeLock(android.os.PowerManager.PARTIAL_WAKE_LOCK, "AgentRing:BtKeepAlive")
            wakeLock?.acquire()
        } catch (e: Exception) {
            android.util.Log.w("AgentRingBT", "Could not acquire wake lock", e)
        }
